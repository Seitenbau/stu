 ...
 * @author Alex Ruiz
 */
public class FileAssertSB extends org.fest.assertions.FileAssert
{
  ...
  protected FileAssertSB hasSameContentAsFileFromClasspath(String file)
  {
    if (file == null)
      throw new NullPointerException(formattedErrorMessage("File to compare to should not be null"));
    isNotNull();
    InputStream expected = FileAssertSB.class.getResourceAsStream(file);
    return hasSameContentAs(expected, file);
  }

  protected FileAssertSB hasSameContentAs(InputStream stream, String fileName)
  {
