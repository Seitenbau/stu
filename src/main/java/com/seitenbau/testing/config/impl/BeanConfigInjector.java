...
public class BeanConfigInjector implements ConfigInjector
{
  ...
  /**
   * Injects the property value into the test configuration instance
   * or into the static fields or a static configuration class.
   * 
   * @param values the property value which should be inject, not
   *        null.
   * 
   * @param injetInto Instance of a configuration class or class
   *        object with static fields.
   * 
   * @throws NonStaticFieldException when injetInto is a class and the
   *         class has non static fields with the StoredProperty
   *         annotation.
   */
  ...
  protected void doInjectValues(ValueProvider values, Class<?> clazz, Object instance)
  {
    ...
    for (Field field : fields)
    {
      ...
      {
        throw new NonStaticFieldException("Static class configuration " + clazz.getName()
            + " initialize with non static field : " + field.getName());
      }
  ...
  protected Object transformValue(ValueProvider values, Class<?> type, StoredProperty anno, String errorDetail)
  {
    ...
    else
    {
      ...
      // TODO : NOT_SET_VALUE should actually be == not equals, the
      // DSL parsing seems to be buggy
  ...
  /**
   * don't use this to catch exceptions future Version will only throw
   * an RuntimeException instead
   */
  @Deprecated
  // don't use this to catch exceptions future Version will only throw
  // an RuntimeException instead
  ...
  protected Object parseInputType(Class<?> type, String valueAsString, String fieldName)
  {
    ...
    if (type.isPrimitive())
    {
      System.out.println("ERROR: Configuration Field '" + fieldName + "' is a primitive type: " + type);
      throw new RuntimeException("Configuration Fields cannot be primitives! " + fieldName);
    }
    System.out.println("ERROR: Configuration Field '" + fieldName + "' is of a unknown Type " + type);
