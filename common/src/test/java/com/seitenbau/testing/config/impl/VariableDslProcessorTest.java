...
public class VariableDslProcessorTest extends TestCase {
	...
	public void testVerschachteltesReplacement() {
		verify("notFound${rainer}$${verschachtelt}").isEqualTo(
				"notFoundval-rainer$level2-level1-inner-value");

		verify("notFound${rainer${rainer}}").isEqualTo(
				"notFound${rainerval-rainer}");
		verify("notFound${server-${key03}}").isEqualTo(
				"notFoundval-key03-server");
